...
class Attack:
    ...
    def __init__(self, arch, pruning,t_prune, activation, device='cuda',
                ) -> None:
        if device == "cuda":
            assert torch.cuda.is_available()
        self.device = device
        self.model_name = "KonCept"
        self.arch = arch
        self.prune = pruning
        ...
        # self.prune 
        self.model = normalize_model(KonCept512(num_classes=1, activation=self.activation),
                                     mean=[0.5,0.5,0.5],
                                     std=[0.5,0.5,0.5]).to(self.device)
        ic(self.model)
        # self.model.eval()
    ...
    def compute_output(self, x):
        print(self.model(x), x.shape)
        out = self.model(normalize(x, [0.5, 0.5, 0.5], [0.5, 0.5, 0.5]))
        print(out, out.item())
        return out.item()

    def load_checkpoints(self, checkpoints_path="KonCept.pth"):
    ...
    def save_vals_to_file(self, csv_results_dir='.'):
        ...
        arch_status = f'{self.arch}{prune}+{activation}'
...
class TestLoader(Dataset):
    def __init__(self, dataset, dataset_path, resize=False, crop=False, resize_size_h=498, resize_size_w=664, **kwargs):
        ...
        if self.dataset == 'NIPS':
            ...
            status ='test'
            self.im_names = Info[Info['set']==status]['image_name'].to_numpy()
        else:
