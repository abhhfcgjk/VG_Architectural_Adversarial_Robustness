...
class IQALoss(nn.Module):
    ...
    def loss_func(self, y_pred, y):
        ...
        if self.monotonicity_regularization:
            loss += self.gamma * monotonicity_regularization(
                y_pred, y, detach=self.detach
            )
        return loss
...
def monotonicity_regularization(y_pred, y, detach=False):
    ...
    if y_pred.size(0) > 1:  #
        ...
        scale = (
            1 + torch.max(ranking_loss.detach())
            if detach
            else 1 + torch.max(ranking_loss)
        )
...
def norm_loss_with_normalization(
    ...
    if batch_size > 1:
        ...
        y_pred = y_pred - m_hat
        normalization = (
            torch.norm(y_pred.detach(), p=q) if detach else torch.norm(y_pred, p=q)
        )
        ...
        if alpha[0] > 0:
            ...
            if p < 1:
                err += eps
            loss0 = (
                torch.norm(err, p=p) / scale
            )  # Actually, p = q = 2 is related to PLCC
        ...
        if alpha[1] > 0:
            ...
            loss1 = (
                torch.norm(err, p=p) / scale
            )  # Actually, p = q = 2 is related to LSR
...
def norm_loss_with_scaling(y_pred, y, alpha=[1, 1], p=2, detach=False):
    if y_pred.size(0) > 1:
        normalization = (
            torch.norm(y_pred.detach(), p=p) if detach else torch.norm(y_pred, p=p)
        )
