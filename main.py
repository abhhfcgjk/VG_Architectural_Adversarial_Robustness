...
class MetricModel(torch.nn.Module):
    def __init__(self, dev, model_path=PATH, batch_size=8):
        ...
        super().__init__()
        device = dev
        self.model = IQAModel(arch=args.architecture, pool=args.pool, use_bn_end=args.use_bn_end, P6=args.P6, P7=args.P7).to(device)
...
def test_attack(attack_callback, model, dataset_path='./NIPS_test/', checkpoints_path='LinearityIQA/checkpoints/p1q2.pth',
                 attack_type='FGSM', device_='cpu', csv_results_dir='.'):
    ...
    for image_path in tqdm(Path(dataset_path).iterdir(), total=len([x for x in Path(dataset_path).iterdir()])):
        ...
        for eps in epsilons:
            im_attacked = attack_callback(im, model=model, metric_range=100, device=device_, eps=eps)
            ...
            with torch.no_grad():
                ...
                results.loc[len(results.index)] = [Path(image_path).stem, np.mean(clear_val), np.mean(attacked_val)]
...
if __name__ == "__main__":
    ...
    model = IQAModel(arch=args.architecture, pool=args.pool, use_bn_end=args.use_bn_end, P6=args.P6, P7=args.P7).to(args.device)
