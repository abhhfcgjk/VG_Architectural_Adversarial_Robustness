...
class VGGBackEnd(nn.Module):
    def __init__(self, features: nn.Module, num_classes=1000, init_weights: bool = True, dropout: float = 0.5) -> None:
        super().__init__()
...
def _vgg(cfg: str, batch_norm, weights, progress, **kwargs) -> VGGBackEnd:
    if weights is not None:
        kwargs["init_weights"] = False
        if weights.meta["categories"] is not None:
            _ovewrite_named_param(kwargs, "num_classes", len(weights.meta["categories"]))
    model = VGGBackEnd(make_layers(cfgs[cfg], batch_norm=batch_norm), **kwargs)
    if weights is not None:
