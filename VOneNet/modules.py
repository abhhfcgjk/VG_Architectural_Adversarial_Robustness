...
class OneNet(nn.Module):
    def __init__(self, vone_block, bottleneck, model_back_end):
        super().__init__()
        ...
        self.layers = [self.model[0][-1].layer1, self.model[0][-1].layer2, self.model[0][-1].layer3, self.model[0][-1].layer4]
    ...
    def __next__(self):
        if self.index < 2:
            self.index += 1
            return self.model[0][self.index-1]
        else:
            self.index += 1
...
class VOneBlock(nn.Module):
    ...
    def gabors_f(self, x):
        ...
        # print(s_q0[:, self.simple_channels:, :, :])

        v1 = torch.clamp(s_q0[:, self.simple_channels:, :, :].clone(),  min=10e-8) #+ torch.clamp(s_q0[:, self.simple_channels:, :, :].clone(),  max=-10e-2)
        v2 = torch.clamp(s_q1[:, self.simple_channels:, :, :].clone(), min=10e-8) #+ torch.clamp(s_q1[:, self.simple_channels:, :, :].clone(), max=-10e-2)
        val =  v1** 2 + v2 ** 2
        
        c = self.complex(torch.sqrt(val)/np.sqrt(2))
