...
class PLSGPU(nn.Module):
    """
    Partial Least Squares GPU.

    Parameters
    ----------
    pls : sklearn PLS model
        Pre-trained PLS model from sklearn with attributes like x_rotations_, x_mean_, x_std_.

    batch_size : int or None, (default=32)
        The number of samples to be sent for GPU computation.
    """
    def __init__(self, pls=None, batch_size=32):
        super(PLSGPU, self).__init__()
        self.batch_size = batch_size

        # Extract parameters from the sklearn PLS model
        self.x_mean = torch.tensor(pls.x_mean_, dtype=torch.float32).cuda()
        self.x_std = torch.tensor(pls.x_std_, dtype=torch.float32).cuda()
        self.weights = torch.tensor(pls.x_rotations_, dtype=torch.float32).cuda()

    def forward(self, X):
        """Forward pass to transform data using the PLS model."""
        # Normalize the input data
        X_normalized = (X - self.x_mean) / self.x_std
        # Project the data onto the latent space
        X_transformed = torch.matmul(X_normalized, self.weights)
    ...
    def transform(self, X):
        """
        Transform the data using the GPU-enabled PLS model.
        ...
        with torch.no_grad():
            for batch in dataloader:
                batch_data = batch[0]  # Extract the batch data
                transformed = self.forward(batch_data)
                transformed_batches.append(transformed)

        return torch.cat(transformed_batches, dim=0)  # Combine and move to CPU
