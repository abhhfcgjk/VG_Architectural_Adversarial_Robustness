...
class PLSRegressionCUDA:
    ...
    def fit(self, X, Y):
        E = self.cudaf(X - X.mean(dim=0)).to(torch.float64)
        F = self.cudaf(Y - Y.mean(dim=0)).to(torch.float64)
        # X = TP^t + E
        T = self.cudaf(torch.zeros((X.size(0), self.n_components))).to(torch.float64)
        P = self.cudaf(torch.zeros((X.size(1), self.n_components))).to(torch.float64)
        W = self.cudaf(torch.zeros((X.size(1), self.n_components))).to(torch.float64)
        # Y = UQ^t + F
        U = self.cudaf(torch.zeros((Y.size(0), self.n_components))).to(torch.float64)
        Q = self.cudaf(torch.zeros((Y.size(1), self.n_components))).to(torch.float64)
        # U = BT
        B = self.cudaf(torch.zeros((self.n_components))).to(torch.float64)
