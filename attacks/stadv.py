...
def metric_d(model, x0, x1=None, k=None, b=None, maxval=METRIC_MAX_VAL):
    # d0 = model.forward(x0)
    d0 = model(x0)[-1] * k[0] + b[0]
    if x1 is None:
        return d0, maxval * 1.5  # metric-dependent const
    # d1 = model.forward(x1)
    d1 = model(x1)[-1] * k[0] + b[0]
    return d0, d1
...
def func(
    ...
    s_adv, s_other = metric_d(model, pert_out, x_other, k, b, maxval)
...
def attack(compress_image, model=None, k=None, b=None, metric_range=100, device="cpu"):
    MAXITER = 3
    ALPHA = 50
    BETA = 1e-5
    ...
    def closure():
        ...
        s_adv, s_other = metric_d(
            model, pert_out, None, k, b, maxval=metric_range
        )
